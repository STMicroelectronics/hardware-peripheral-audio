...
#define BIT_EQUAL(mask, val1, val2)     (((val1) & (mask)) == ((val2) & (mask)))
...
{
  enum mixer_ctl_type ctl_type = MIXER_CTL_TYPE_UNKNOWN;
  ...
  if (!ctl) {
     ...
     * and try again
     */
    card = cm->mixer_card_number;
    mixer_close(cm->mixer);
    cm->mixer = mixer_open(card);

    if (!cm->mixer) {
      ALOGE("Failed to re-open mixer card %u", card);
      return -EINVAL;
    }
  ...
  switch(ctl_type) {
    ...
    case MIXER_CTL_TYPE_INT:
      ...
      ALOGE_IF((ctl_type == MIXER_CTL_TYPE_BOOL) && (pctl->value.uinteger > 1),
...
static void apply_ctls_l(struct config_mgr *cm, struct ctl *pctl,
                         const int ctl_count)
{
  struct mixer_ctl *ctl = NULL;
  unsigned int vnum = 0;
  ...
  int err = 0;
  uint8_t ctl_data[BYTE_ARRAY_MAX_SIZE] = { 0 };

  ALOGV("+apply_ctls_l");

  for (int i = 0; i < ctl_count; ++i, ++pctl) {
    ...
    switch (mixer_ctl_get_type(ctl)) {
      ...
      case MIXER_CTL_TYPE_INT:
        ...
        ALOGV("apply ctl '%s' = 0x%x (%d values)", mixer_ctl_get_name(ctl),
                                                   pctl->value.uinteger,
                                                   value_count);
      ...
      case MIXER_CTL_TYPE_BYTE:
        ...
        ALOGV("apply ctl '%s' = byte data (%d bytes)", mixer_ctl_get_name(ctl),
                                                       vnum);
                                ...
                                int first_id, int second_id)
{
  ...
  ALOGV("Applying paths [first=%u second=%u] to device(@%p, mask=0x%x '%s')",
            first_id, second_id, ppath, pdev->type,
            debug_device_to_name(pdev->type));
                                     ...
                                     int first_id, int second_id)
{
  ...
  ALOGV("Apply paths [first=%u second=%u] to devices in 0x%x", first_id,
                                                               second_id,
                                                               devices);
...
{
  struct mixer_ctl *ctl = volctl->handle;
  uint val = 0;
  uint range = 0;
...
int set_hw_volume(const struct hw_stream *stream, int left_pc, int right_pc)
{
  ...
  if (s->controls.volume_left.handle != NULL) {
    if (s->controls.volume_right.handle == NULL) {
...
static struct stream *find_named_stream(struct config_mgr *cm, const char *name)
{
  struct stream *s = cm->stream_array.streams;

  for (int i = cm->stream_array.count - 1; i >= 0; --i) {
    if (s->name) {
...
static struct device *hal_device_to_alsa(const struct config_mgr *cm,
                                         const audio_devices_t devices)
{
  struct device *alsa_device = NULL;
  struct device *aux = NULL;

  /* find an alsa device matching the one requested */
  for (size_t i = 0; i < cm->device_array.count; ++i) {
    aux = cm->device_array.devices + i;

    /* Try to find a matching device type (in / out) and a default device if
     * requested or the matching requested device.
     **/
    if (aux->type != 0 &&
        BIT_EQUAL(AUDIO_DEVICE_BIT_IN, aux->type, devices) &&
          ((AUDIO_DEVICE_BIT_DEFAULT & aux->type & devices) ||
           BIT_EQUAL(~(AUDIO_DEVICE_BIT_IN | AUDIO_DEVICE_BIT_DEFAULT),
                     aux->type, devices))) {
      ALOGV("get_stream found a matching device: %x", aux->type);
      if (alsa_device) {
        ALOGE("get_stream found multiple devices for %x", devices);
        return NULL;
      }
      alsa_device = aux;
    }
                                   ...
                                   const struct audio_config *config)
{
  ...
  d = hal_device_to_alsa(cm, devices);

  if (!d) {
    ALOGE("get_stream didn't find any device for %x", devices);
    return NULL;
  }
  ...
  /* look for stream associated to the device found */
  pthread_mutex_lock(&cm->lock);
  for (i = cm->stream_array.count - 1; i >= 0; --i) {
    ALOGV("get_stream: require type=%d, device=%d; try type=%d device=%d"
            " refcount=%d refmax=%d", type, d->device_number, s[i].info.type,
            s[i].info.device_number, s[i].ref_count, s[i].max_ref_count);

    if (d->device_number == s[i].info.device_number && s[i].info.type == type) {
  ...
  if (i >= 0) {
    ALOGV("-get_stream =%p (refcount=%d)", &s[i].info, s[i].ref_count);
    return &s[i].info;
  }
                                         ...
                                         const char *name)
{
  ...
  struct stream *s = find_named_stream(cm, name);;
  ...
  if (s != NULL) {
    ...
    ALOGE("-get_named_stream no suitable stream: %s", name);
    return NULL;
  }
}
...
static const struct parse_element elem_table[e_elem_count] = {
  ...
  [e_elem_device] =    {
    .name = "device",
    .valid_attribs = BIT(e_attrib_name) | BIT(e_attrib_device) |
                     BIT(e_attrib_default),
    .required_attribs = BIT(e_attrib_name),
...
static struct stream* new_stream(struct dyn_array *array, struct config_mgr *cm)
{
  ...
  s->disable_path = -1;
  s->controls.volume_left.handle = NULL;
  s->controls.volume_right.handle = NULL;
  return s;
}
...
static int add_path_name(struct parse_state *state, const char *name)
{
  struct dyn_array *array = &state->path_name_array;
  const char *s = NULL;
  int index = find_path_name(state, name);  /* Check if already in array */
...
static int parse_path_start(struct parse_state *state)
{
  ...
  struct path *path = NULL;
  int id = add_path_name(state, name);
...
static int parse_enable_disable_start(struct parse_state *state, bool is_enable)
{
  ...
  int i = find_path_name(state,path_name);
  ...
  if (is_enable) {
    ALOGV("Add enable path '%s' (id=%d)", state->path_name_array.path_names[i],
                                          i);
    state->current.stream->enable_path = i;
  } else {
    ALOGV("Add disable path '%s' (id=%d)", state->path_name_array.path_names[i],
                                           i);
...
static int parse_stream_ctl_start(struct parse_state *state)
{
  ...
  const char *index = state->attribs.value[e_attrib_index];
  struct mixer_ctl *ctl = NULL;
  struct stream_control *streamctl = NULL;
  uint idx_val = 0;
  uint32_t v = 0;
  int r = 0;
  ...
  if (0 == strcmp(function, "leftvol")) {
    ALOGE_IF(state->current.stream->controls.volume_left.handle != NULL,
                "Left volume control specified again");
    streamctl = &(state->current.stream->controls.volume_left);
  } else if (0 == strcmp(function, "rightvol")) {
    ALOGE_IF(state->current.stream->controls.volume_right.handle != NULL,
...
static int parse_device_start(struct parse_state *state)
{
  ...
  uint32_t alsa_device = 0xFFFFFFFF;
  uint32_t default_device = 0;
  uint32_t *existing_devices = NULL;
  const struct parse_device *p = NULL;
  struct device *d = NULL;
  ...
  if (device_flag != 0) {
    ...
    if ((device_flag & *existing_devices) == device_flag) {
      ...
      ALOGE("Device = 0x%x extisting_devices = 0x%x", device_flag,
                                                      *existing_devices);
  ...
  if (attrib_to_uint(&alsa_device, state, e_attrib_device) == -EINVAL) {
    return -EINVAL;
  }

  if (attrib_to_uint(&default_device, state, e_attrib_default) == -EINVAL) {
    return -EINVAL;
  }

  if (default_device) {
    device_flag |= AUDIO_DEVICE_BIT_DEFAULT;
  }

  ALOGV("Add device '%s' = %x (%x)", dev_name, alsa_device, device_flag);

  d = new_device(array, device_flag, alsa_device);
  if (d == NULL) {
    return -ENOMEM;
  }
...
static int parse_mixer_start(struct parse_state *state)
{
  ...
  state->cm->mixer_card_number = card;
  ...
  state->stack.entry[state->stack.index - 1].valid_subelem = BIT(e_elem_device)
                                                           | BIT(e_elem_stream);
  return 0;
}
                                ...
                                const XML_Char **attribs)
{
  ...
  const uint32_t valid_elems = state->stack.entry[stack_index].valid_subelem;
  int i = 0;
  ...
  if ((i >= e_elem_count) || (stack_index >= MAX_PARSE_DEPTH)) {
    ...
    state->stack.entry[stack_index].valid_subelem = elem_table[i].valid_subelem;
...
{
  const struct dyn_array *path_array = NULL;
  const struct dyn_array *ctl_array = NULL;
  const struct device *dev = NULL;
  const struct ctl *c = NULL;
  uint dev_idx = 0;
  uint path_idx = 0;
  uint ctl_idx = 0;
  ...
  for (dev_idx = 0; dev_idx < cm->device_array.count; dev_idx++) {
    ...
    ALOGV("Device %d: type 0x%x, %d paths", dev_idx, dev->type,
                                            path_array->count);
    for (path_idx = 0; path_idx < path_array->count; path_idx++) {
      ...
      for (ctl_idx = 0; ctl_idx < ctl_array->count; ctl_idx++) {
        ...
        ALOGV("Ctl %d: name %s, index %d, array_count %d, type %d ",
                  ctl_idx, c->name, c->index, c->array_count, c->type);
...
struct config_mgr *init_audio_config()
{
  ...
  pthread_mutex_init(&mgr->lock, (const pthread_mutexattr_t *) NULL);
...
static void free_usecases(struct stream *stream)
{
  ...
  for (; uc_count > 0; uc_count--, puc++) {
    free((void *)puc->name);
    pcase = puc->case_array.cases;
    for (int i = puc->case_array.count; i > 0; i--, pcase++) {
